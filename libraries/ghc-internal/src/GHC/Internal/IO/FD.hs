...
import GHC.Internal.Event.Thread (threadWaitRead, threadWaitWrite, closeFdWith)
...
close fd =
  do let closer realFd =
           throwErrnoIfMinus1Retry_ "GHC.Internal.IO.FD.close" $
#if defined(mingw32_HOST_OS)
...
setSize fd size =
  throwErrnoIf_ (/=0) "GHC.Internal.IO.FD.setSize" $
...
dup :: FD -> IO FD
dup fd = do
  newfd <- throwErrnoIfMinus1 "GHC.Internal.IO.FD.dup" $ c_dup (fdFD fd)
  return fd{ fdFD = newfd }

dup2 :: FD -> FD -> IO FD
dup2 fd fdto = do
  -- Windows' dup2 does not return the new descriptor, unlike Unix
  throwErrnoIfMinus1_ "GHC.Internal.IO.FD.dup2" $
...
ready fd write msecs = do
  r <- throwErrnoIfMinus1Retry "GHC.Internal.IO.FD.ready" $
...
setEcho :: FD -> Bool -> IO ()
setEcho fd on = GHC.Internal.System.Posix.Internals.setEcho (fdFD fd) on

getEcho :: FD -> IO Bool
getEcho fd = GHC.Internal.System.Posix.Internals.getEcho (fdFD fd)

setRaw :: FD -> Bool -> IO ()
setRaw fd raw = GHC.Internal.System.Posix.Internals.setCooked (fdFD fd) (not raw)
...
fdRead fd ptr _offset bytes
  = do { r <- readRawBufferPtr "GHC.Internal.IO.FD.fdRead" fd ptr 0
...
fdReadNonBlocking fd ptr _offset bytes = do
  r <- readRawBufferPtrNoBlock "GHC.Internal.IO.FD.fdReadNonBlocking" fd ptr
...
fdWrite fd ptr _offset bytes = do
  res <- writeRawBufferPtr "GHC.Internal.IO.FD.fdWrite" fd ptr 0
...
fdWriteNonBlocking fd ptr _offset bytes = do
  res <- writeRawBufferPtrNoBlock "GHC.Internal.IO.FD.fdWriteNonBlocking" fd ptr 0
            (fromIntegral $ clampWriteSize bytes)
