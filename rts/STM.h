...
/* Create and enter a new transaction context. `outer` must be NO_TREC. */

StgTRecHeader *stmStartTransaction(Capability *cap, StgTRecHeader *outer);

/*
 * Roll back the current transaction attempt and remove wait-queue entries.
 */

void stmAbortTransaction(Capability *cap, StgTRecHeader *trec);
void stmFreeTransaction(Capability *cap, StgTRecHeader *trec);
...
/*----------------------------------------------------------------------
   ...
   Commit/registration operations
   ------------------------------

   Operations that return boolean results should be interpreted as:
   ...
   may cause needless spinning instead of blocking (in the case of wait).

   In defining the behaviour of waiting we distinguish between two different
   aspects of a thread's runnability:
   ...
   and for waiting to return false even when the previous transaction context
